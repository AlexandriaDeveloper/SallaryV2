...
using Application.EmployeesForms.Commands.UploadSubscriptionElement;
using Application.FormEmployees.Queries.NewFolder;

namespace Web.Api.Controllers
{
    public class FormEmployeeController : BaseController
    {
        public FormEmployeeController(IMediator mediator) : base(mediator)
        {
        }
        ...
        [HttpGet("GetFormEmployeeByFormId")]
        public async Task<ActionResult<Result<Unit>>> GetFormEmployeeByFormId([FromQuery] GetEmployeeByFormIdQueryParam param)
        {

            var result = await Mediator.Send(new GetFormEmployeeByFormIdAndEmployeeIdQuery(param));
            if (result.IsFailure)
            {
                return HandleFailureResult(result);
            }
        ...
        [HttpGet("GetEmployeesByEmployeeIdAndFormId")]
        public async Task<ActionResult<Result<Unit>>> GetEmployeesByEmployeeIdAndFormId([FromQuery] FormEmployeeQueryParam param)
        {

            var result = await Mediator.Send(new GetFormEmployeeByEmployeeIdAndFormIdQuery(param));
            if (result.IsFailure)
            {
                return HandleFailureResult(result);
            }
